            ...
            <ul className="stores-list">
                ...
                <li key={store._id}>
                    <label htmlFor={`store_${store.name}`}> {capitalize(store.name)}:</label>
                        <input type='number' name={store._id} min={0} step={1} onChange={props.handleInputChange} id={`store_${store.name}`} value={props.product.quantities && props.product.quantities[props.product.quantities.map((val)=>val.store).indexOf(store._id)].quantity || ''}/>
                </li>)}
            </ul>
            </section>
